...
export default defineStore("useExecutionStore", () => {
  ...
  async function encerrar_execucao(id_execucao: string) {
    botNs.emit("bot_stop", { id_execucao: id_execucao });
  }

  async function download_execucao(id_execucao: string) {
  ...
  watch(execucao, (newV) => {
    execucaoBot.value = newV?.id_execucao as string;
    logs.value = [];
    if (execucaoBot.value) {
